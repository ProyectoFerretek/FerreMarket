...
import { 
  ...
  Shield, ShieldCheck, User, Users, Eye, Calendar,
...
const GestionUsuarios: React.FC = () => {
  ...
  const [ordenamiento, setOrdenamiento] = useState<{ campo: keyof Usuario; direccion: 'asc' | 'desc' }>({
    campo: 'fechaCreacion',
    direccion: 'desc'
  });
  ...
  const usuariosEnPagina = usuariosFiltrados.slice((pagina - 1) * itemsPorPagina, pagina * itemsPorPagina);
  ...
  const handleGuardarUsuario = (usuarioData: UsuarioFormData) => {
    if (usuarioSeleccionado) {
      console.log('Actualizando usuario:', usuarioSeleccionado.id, usuarioData);
    } else {
  ...
  const getUltimoAccesoEstado = (ultimoAcceso?: string) => {
    if (!ultimoAcceso) return { texto: 'Nunca', color: 'text-gray-500', icono: <XCircle size={14} /> };
    ...
    if (diferenciaDias === 0) {
      return { texto: 'Hoy', color: 'text-green-600', icono: <CheckCircle size={14} /> };
    } else if (diferenciaDias <= 7) {
      return { texto: `Hace ${diferenciaDias} día${diferenciaDias > 1 ? 's' : ''}`, color: 'text-yellow-600', icono: <Clock size={14} /> };
    } else {
      return { texto: `Hace ${diferenciaDias} días`, color: 'text-red-600', icono: <AlertTriangle size={14} /> };
    }
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center">
            <ShieldCheck size={20} className="text-purple-600 mr-3" />
            <div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">{estadisticas.conectadosHoy}</p>
              <p className="text-xs text-gray-600">Conectados hoy</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th 
                  className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
                  onClick={() => handleOrdenar('ultimoAcceso')}
                >
                  <div className="flex items-center">
                    Último Acceso
                    <ArrowUpDown size={14} className="ml-1" />
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {usuariosEnPagina.map(usuario => {
                const ultimoAccesoInfo = getUltimoAccesoEstado(usuario.ultimoAcceso);
                ...
                return (
                  <tr key={usuario.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className={`ml-2 px-2 py-1 text-xs font-semibold rounded-full ${
                          usuario.rol === 'admin' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                        }`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <span className={`${ultimoAccesoInfo.color} mr-2`}>
                          {ultimoAccesoInfo.icono}
                        </span>
                        <span className={`text-sm ${ultimoAccesoInfo.color}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        ...
                        {puedeRealizarAccion('eliminar') && usuario.id !== '1' && (
                          <button
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-gray-900 truncate">{usuario.nombre}</h3>
                  ...
                  <div className="mt-2 text-xs text-gray-500 flex items-center">
                    ...
                    Creado: {formatFecha(usuario.fechaCreacion)}
                  </div>
                  
                  <div className="mt-1 text-xs flex items-center">
                    <span className={`${ultimoAccesoInfo.color} mr-1`}>
                      {ultimoAccesoInfo.icono}
                    </span>
                    <span className={ultimoAccesoInfo.color}>
                      Último acceso: {ultimoAccesoInfo.texto}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 px-4 py-3">
          <div className="flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-0">
            <div className="text-sm text-gray-700 text-center sm:text-left">
              Mostrando {((pagina - 1) * itemsPorPagina) + 1} a {Math.min(pagina * itemsPorPagina, usuariosFiltrados.length)} de {usuariosFiltrados.length} usuarios
            </div>
            <div className="flex items-center space-x-2">
              <button
